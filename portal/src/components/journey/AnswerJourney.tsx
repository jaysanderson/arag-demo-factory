...
import {
  X, Play, Pause, ChevronLeft, ChevronRight, Sparkles, ExternalLink,
  Quote, CheckCircle2, Compass, Loader2, Volume2, VolumeX,
} from 'lucide-react';
import { buildStops, relate, type JourneyStop } from '../../lib/journey';
import { ConfidenceRing, matchColor } from './ConfidenceRing';
import { cleanTitle, gradientFor, safeHost } from '../../lib/util';
import { renderMarkdown } from '../../lib/markdown';

// Auto-advance dwell after a stop's relation finishes (covers the slow read-scroll).
const DWELL_MS = 5600;

interface Rel { text: string; done: boolean }

/**
 * Cinematic, auto-playing walk through the grounding behind an answer: strongest
 * match first, a confidence ring per source, and a live resource-scoped "how does
 * this relate?" answer. It turns the citation list into a designed experience —
 * the clearest way to show that an answer is grounded, not generated.
 */
export function AnswerJourney({ open, query, filters = [], citedIds = [], onClose }: {
  open: boolean; query: string; filters?: string[]; citedIds?: string[]; onClose: () => void;
}) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const s = await buildStops(query, filters, citedIds);
  ...
  // Stream the "how this relates" answer for the current stop (cached per stop).
  useEffect(() => {
    if (!open || !atStop || rel[idx]) return;
    const s = stops[idx];
    setRel((r) => ({ ...r, [idx]: { text: '', done: false } }));
    const abort = relate(s.resourceId, query, {
      onToken: (acc) => setRel((r) => ({ ...r, [idx]: { text: acc, done: false } })),
      onDone: () => setRel((r) => ({ ...r, [idx]: { text: r[idx]?.text || '', done: true } })),
    });
    return () => abort();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [idx, open, atStop]);
  ...
  // Once a stop's relation has streamed in, slowly scroll through the text
  // (cinematic reveal). Cancels if the viewer scrolls manually.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    return () => synth.cancel();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, narrate, atStop, curDone, idx, relText]);
  // Always stop speaking when the overlay unmounts.
  useEffect(() => () => { try { window.speechSynthesis?.cancel(); } catch { /* */ } }, []);

  if (!open) return null;

  return createPortal((
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-ink-950/85 p-4 backdrop-blur-sm animate-fade-in-fast" onClick={onClose}>
      <div
        className="relative flex max-h-[88vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-ink-900 text-white shadow-xl ring-1 ring-white/10"
      ...
      >
        ...
        <div className="absolute inset-x-0 top-0 z-20 flex items-center justify-between px-4 py-3">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-black/40 px-2.5 py-1 text-[11px] font-semibold backdrop-blur-sm">
            <Compass size={13} className="text-accent" /> {atStop ? `Source ${idx + 1} of ${n}` : 'Answer journey'}
          </span>
        ...
        <div ref={contentRef} className="min-h-0 flex-1 overflow-y-auto scroll-slim">
          {loading ? (
            <div className="flex h-80 flex-col items-center justify-center gap-3 text-white/70"><Loader2 size={26} className="animate-spin text-accent" /><p className="text-sm">Tracing the grounding…</p></div>
          ) : n === 0 ? (
            <div className="flex h-80 flex-col items-center justify-center gap-3 px-8 text-center text-white/70">
              ...
              <button onClick={onClose} className="btn btn-ghost !border-white/20 !text-white hover:!bg-white/10">Close</button>
            </div>
          ) : idx === -1 ? (
            /* INTRO */
            <div className="flex min-h-[26rem] flex-col items-center justify-center gap-4 px-8 py-12 text-center">
              <span className="flex h-14 w-14 items-center justify-center rounded-2xl animate-fade-up" style={{ background: 'color-mix(in srgb, var(--brand) 28%, transparent)', color: 'var(--accent)' }}><Sparkles size={26} /></span>
              <h2 className="animate-fade-up font-display text-3xl font-semibold text-white">How this answer was built</h2>
              <p className="max-w-md animate-fade-up text-sm text-white/70">
              ...
              <p className="max-w-md animate-fade-up rounded-lg bg-white/5 px-4 py-2 text-sm italic text-white/80">“{query}”</p>
              <button onClick={() => goTo(0)} className="btn btn-brand mt-1 animate-fade-up"><Play size={15} /> Begin the journey</button>
            </div>
          ) : idx >= n ? (
            /* OUTRO */
            <div className="flex min-h-[26rem] flex-col items-center justify-center gap-4 px-8 py-12 text-center">
              <span className="flex h-14 w-14 items-center justify-center rounded-2xl animate-fade-up" style={{ background: 'color-mix(in srgb, var(--brand) 28%, transparent)', color: 'var(--accent)' }}><CheckCircle2 size={26} /></span>
              <h2 className="animate-fade-up font-display text-3xl font-semibold text-white">That's the trail</h2>
              <p className="max-w-md animate-fade-up text-sm text-white/70">Your answer was synthesized from these {n} sources, strongest first:</p>
              <div className="w-full max-w-md animate-fade-up space-y-1.5">
                {stops.map((s, i) => (
                  <button key={s.resourceId} onClick={() => goTo(i)} className="flex w-full items-center gap-2 rounded-lg bg-white/5 px-3 py-2 text-left hover:bg-white/10">
                    <span className="h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: matchColor(s.score) }} />
              ...
              <div className="mt-1 flex animate-fade-up gap-2">
                <button onClick={() => { setRel({}); goTo(-1); }} className="btn btn-ghost !border-white/20 !text-white hover:!bg-white/10"><Play size={14} /> Replay</button>
                <button onClick={onClose} className="btn btn-brand">Done</button>
              </div>
            </div>
          ) : stop ? (
            /* STOP */
            <div>
              {/* hero */}
              <div className="relative h-52 w-full overflow-hidden" style={{ background: gradientFor(stop.title) }}>
                <div className="absolute inset-0 bg-gradient-to-t from-ink-900 via-ink-900/50 to-transparent" />
                {stop.cited && (
                  <span className="absolute right-4 top-14 inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-semibold text-white shadow-sm" style={{ background: 'color-mix(in srgb, var(--brand) 90%, black)' }}><CheckCircle2 size={12} /> Cited in the answer</span>
                )}
              ...
              <div className="space-y-4 p-6">
                ...
                <div className="animate-fade-up">
                  <div className="mb-1 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-accent"><Sparkles size={12} /> How this relates to your question</div>
                  {relText ? (
                    <div className="text-sm leading-relaxed text-white/90 [&_p]:mb-2 [&_strong]:text-white [&_a]:text-accent" dangerouslySetInnerHTML={{ __html: renderMarkdown(relText) }} />
                  ) : (
                ...
                {stop.url && (
                  <a href={stop.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-sm font-medium text-accent hover:opacity-80">Open original <ExternalLink size={13} /></a>
                )}
                <Link to={`/r/${stop.resourceId}`} onClick={onClose} className="ml-3 inline-flex items-center gap-1 text-sm font-medium text-white/60 hover:text-white">View in library →</Link>
              </div>
            </div>
          ) : null}
        </div>
        ...
        {!loading && n > 0 && (
          <div className="border-t border-white/10 bg-ink-900/95 px-4 py-3">
            <div className="mb-2 h-1 overflow-hidden rounded-full bg-white/10">
              <div className="h-full rounded-full bg-accent" style={{ width: idx < 0 ? '0%' : idx >= n ? '100%' : `${Math.round(progress * 100)}%`, transition: 'width 0.1s linear' }} />
            </div>
              ...
              <div className="flex flex-1 items-center justify-center gap-1.5">
                {stops.map((s, i) => (
                  <button key={s.resourceId} onClick={() => goTo(i)} aria-label={`Source ${i + 1}`}
                    className={`h-2 rounded-full transition-all ${i === idx ? 'w-6' : 'w-2 bg-white/25 hover:bg-white/50'}`}
                    style={i === idx ? { backgroundColor: matchColor(s.score) } : undefined} />
                ))}
              </div>
