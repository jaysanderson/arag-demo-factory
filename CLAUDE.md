...
- **Ask for the KendoReact license key at the start, and no public npm.** KendoReact without a license
  shows a trial banner in front of the customer — if none is configured (`KENDO_UI_LICENSE` in `.env`
  or a `telerik-license.txt` at the root), ask the SE for their key up front and write it to `.env`;
  the build activates it offline via `scripts/kendo-license.mjs` (using the already-installed
  `@progress/kendo-licensing`). **All npm installs resolve through the Progress HAR registry (the
  `.npmrc`), never npmjs.org** — don't add a dependency HAR can't serve; if something truly isn't on
  HAR, vendor it or find a non-npm path rather than using the public registry.
